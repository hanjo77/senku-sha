...
class User {
    ...
    function login() {
		...
		$return = null; 
		$db_util = new DBUtil();
		$query = "SELECT `id` FROM `user` WHERE (`name` = '".mysql_real_escape_string($_POST['name'])."' OR `email` = '".mysql_real_escape_string($_POST['name'])."') AND `password` = PASSWORD('".mysql_real_escape_string($_POST['password'])."') AND `active` = 1";
		echo($query);
